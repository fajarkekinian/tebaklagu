...
public class DaerahScene extends BaseScene implements IOnMenuItemClickListener{

	private LaguAdapter db;
	private Text level1, level2, level3, level4;
	private MenuScene menuChildScene;
	private final String kategori = "daerah";
	...
	private void create(){
		...
		daerah = new Sprite(0, 0, resourcesManager.regiondaerahhead, vbom);
		lvl1 = new ScaleMenuItemDecorator(new SpriteMenuItem(no1, resourcesManager.regiondaerah1, vbom), 1.1f, 1);
		level1 = new Text(0, 0, resourcesManager.helvetica, ""+db.getDaerahLevelstatus(1)+"/"+db.getDaerahLevel(1), vbom);
		
		if (db.getDaerahLevelstatus(1) == db.getDaerahLevel(1)) {
			lvl2 = new ScaleMenuItemDecorator(new SpriteMenuItem(no2, resourcesManager.regiondaerah2, vbom), 1.1f, 1);
			level2 = new Text(0, 0, resourcesManager.helvetica, ""+db.getDaerahLevelstatus(2)+"/"+db.getDaerahLevel(2), vbom);	
		}else{
		...
		if (db.getDaerahLevelstatus(2) == db.getDaerahLevel(2)) {
			lvl3 = new ScaleMenuItemDecorator(new SpriteMenuItem(no3, resourcesManager.regiondaerah3, vbom), 1.1f, 1);
			level3 = new Text(0, 0, resourcesManager.helvetica, ""+db.getDaerahLevelstatus(3)+"/"+db.getDaerahLevel(3), vbom);	
		}else{
		...
		if (db.getDaerahLevelstatus(3) == db.getDaerahLevel(3)) {
			lvl4 = new ScaleMenuItemDecorator(new SpriteMenuItem(no4, resourcesManager.regiondaerah4, vbom), 1.1f, 1);
			level4 = new Text(0, 0, resourcesManager.helvetica, ""+db.getDaerahLevelstatus(4)+"/"+db.getDaerahLevel(4), vbom);	
		}else{
	...
	private void position(){
		CWhead = (int) ((CW - daerah.getWidth()) / 2);
